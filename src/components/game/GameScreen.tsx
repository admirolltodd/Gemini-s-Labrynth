...
export default function GameScreen() {
  ...
  const [isGeneratingPortrait, setIsGeneratingPortrait] = useState(
    !game.portrait,
  );
  ...
  useEffect(() => {
    if (game.companion.loyalty !== prevLoyalty.current) {
      ...
      setLoyaltyMessage({
        value: diff,
        label:
          diff > 0 ? `Loyalty Gained (+${diff})` : `Loyalty Lost (${diff})`,
      });
  ...
  const generatePortrait = async () => {
    try {
      ...
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData) {
          game.setGameState({
            portrait: `data:image/png;base64,${part.inlineData.data}`,
          });
          break;
        }
      }
  ...
  const handleAction = async (action: string) => {
    ...
    try {
      if (!apiKey) {
        throw new Error(
          "VOX-LINK FAILURE: No Gemini API Key provided. Please enter your key in the Settings menu to connect to the Game Master.",
        );
      }
      ...
      game.setGameState({
        hp: {
          current: Math.max(
            0,
            Math.min(game.hp.max, game.hp.current + (updates.hp_change || 0)),
          ),
          max: game.hp.max,
        },
        xp: {
        ...
        companion: {
          ...
          loyalty: Math.max(
            ...
            Math.min(
              100,
              game.companion.loyalty + (updates.loyalty_change || 0),
            ),
          ),
        },
  ...
  const handleSave = async () => {
    const name =
      game.history[0]?.content
        .split(",")[0]
        .replace("Character Created: ", "") || "Unnamed_Operative";
    try {
  ...
  const handleUseItem = async (itemName: string) => {
    ...
    if (itemName.toLowerCase().includes("medkit")) {
      game.setGameState({
        hp: {
          current: Math.min(game.hp.max, game.hp.current + 4),
          max: game.hp.max,
        },
      });
    }
  ...
  const handleCompanionInteract = () => {
    handleAction(
      `I consult with ${game.companion.name || "my companion"} for advice.`,
    );
  ...
  return (
    <div className="flex-1 flex flex-col h-full relative">
      ...
      {/* Main Narrative Area */}
      <div className="flex-1 flex overflow-hidden bg-card/70 backdrop-blur-md">
          ...
          <div className="max-w-3xl mx-auto space-y-8 pb-32">
                  ...
                  <div className="space-y-4 w-full">
                              ...
                              .map((para, pIdx) => (
                                <p
                                  ...
                                  className={cn(
                                    ...
                                    pIdx === 0 &&
                                      "first-letter:text-4xl first-letter:font-serif first-letter:mr-1 first-letter:float-left",
                                  )}
                                >
                                  {para}
                                </p>
                    ...
                    {msg.dialogue && (
                      <motion.div
                        ...
                        className="relative pl-6 py-4 border-l-2 border-primary/30 bg-primary/5 rounded-r-lg group overflow-hidden gothic-border"
                      >
                      ...
                      <div className="flex flex-col gap-3 pt-4 w-full">
                        {Object.entries(msg.choices).map(([key, value]) => (
                          <Button
                            ...
                            variant="outline"
                            disabled={isThinking}
                            className="w-full flex items-start justify-start text-left h-auto py-4 px-5 border-border/80 bg-card/60 hover:border-primary hover:bg-primary/5 transition-all shadow-sm hover:shadow-md hover:shadow-primary/10 group relative overflow-hidden whitespace-normal break-words"
                            onClick={() => handleAction(value)}
                          >
                            <div className="flex gap-4 w-full pr-6">
                              <Badge
                                variant="outline"
                                className="shrink-0 mt-0.5 border-primary/50 text-primary bg-primary/5 uppercase font-bold tracking-widest text-[10px] items-center justify-center h-6 w-10 ring-1 ring-primary/20 group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-primary transition-all shadow-none"
                              >
                                {key}
                              </Badge>
                              <span className="text-sm font-medium leading-relaxed opacity-85 group-hover:opacity-100 flex-1">
                                {value}
                              </span>
                            </div>
                            <ChevronRight className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 -translate-x-2 group-hover:translate-x-0 group-hover:opacity-100 transition-all text-primary w-5 h-5 duration-300" />
                          </Button>
                        ))}
                      </div>
                    )}
            ...
            {isThinking && (
              <motion.div
                ...
                className="relative p-6 border border-primary/30 bg-background/80 backdrop-blur-md rounded-lg overflow-hidden flex flex-col items-center justify-center my-8 gap-4 max-w-lg mx-auto shadow-2xl shadow-primary/10"
              >
                ...
                <div className="relative z-10 flex gap-6 items-center w-full">
                  ...
                  <div className="space-y-2 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="text-xs uppercase tracking-[0.3em] font-bold text-primary">
                        Cogitator Array Active
                      </h3>
                    </div>
                    <p className="text-[11px] text-muted-foreground uppercase tracking-widest font-mono opacity-80 mt-1">
                      {loadingPhrase}
                    </p>
                  </div>
                </div>

                <div className="w-full h-1 bg-secondary rounded-full overflow-hidden mt-2 relative z-10">
                  <motion.div
        ...
        <div className="max-w-4xl mx-auto flex gap-4">
          <div className="flex-1 relative">
            <input
              ...
              placeholder={
                isThinking
                  ? "Waiting for the Warp..."
                  : "Type your command or custom action..."
              }
      ...
      <div className="h-24 border-t border-border bg-background/90 backdrop-blur-md flex items-center px-8 gap-8 z-20">
        ...
        <div className="flex-1 flex items-center justify-between">
          <div className="space-y-1">
            <h3 className="text-xl font-bold parchment-text leading-none uppercase">
              {game.history[0]?.content
                .split(",")[0]
                .replace("Character Created: ", "") || "OPERATIVE"}
            </h3>
          ...
          <div className="flex gap-6">
            <StatHex
              label="HP"
              value={`${game.hp.current}/${game.hp.max}`}
              icon={<Heart size={12} />}
              color="text-red-500"
            />
                ...
                {game.companion.name && (
                  <Badge
                    ...
                    className={cn(
                      ...
                      game.companion.loyalty >= 60
                        ? "text-primary border-primary/30"
                        : game.companion.loyalty < 40
